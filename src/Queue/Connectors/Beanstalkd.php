...
use Hunter\exception\HttpException;
use Hunter\Queue\Connector;
use Hunter\Request;
use Hunter\Queue\Jobs\Beanstalkd as BeanstalkdJob;
use Hunter\Response;
...
class Beanstalkd extends Connector
{
    ...
    {
        $job = new BeanstalkdJob($this, $this->marshalPushedJob(), $this->request->header('beanstalkdJob-message-queue'));
        if ($this->request->header('beanstalkdJob-message-status') == 'success') {
            $job->fire();
        } else {
            $job->failed();
        }
    ...
    {
        return (object) [
            'id'       => $this->request->header('beanstalkdJob-message-id'),
            'payload'  => $this->request->getContent(),
            'attempts' => $this->request->header('beanstalkdJob-message-attempts')
        ];
    }
